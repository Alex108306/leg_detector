...
public:
    OccupancyGridMapping()
        ...
        sync.registerCallback(
            std::bind(&OccupancyGridMapping::laserAndLegCallback, this, std::placeholders::_1, std::placeholders::_2));
...
private:
     ...
     **/
    void laserAndLegCallback(const sensor_msgs::msg::LaserScan::ConstSharedPtr& scan_msg,
                             const leg_detector_msgs::msg::LegArray::ConstSharedPtr& non_leg_clusters) {
        ...
        if (transform_available) {
            ...
            for (std::list<laser_processor::SampleSet*>::iterator c_iter = processor.getClusters().begin();
                ...
                for (laser_processor::SampleSet::iterator s_iter = (*c_iter)->begin(); s_iter != (*c_iter)->end();
                     ++s_iter) {
            ...
            if (transform_succesful) {
                ...
                if (sqrt(pow(grid_centre_pos_x_ - laser_x, 2) + pow(grid_centre_pos_y_ - laser_y, 2)) >
                    shift_threshold_) {
                    ...
                    for (int i = 0; i < width_; i++) {
                        for (int j = 0; j < width_; j++) {
                            ...
                            if (translated_i >= 0 and translated_i < width_ and translated_j >= 0 and
                                translated_j < width_) {
                ...
                for (int i = 0; i < width_; i++) {
                    for (int j = 0; j < width_; j++) {
                        ...
                        double dist = sqrt(
                            pow(i * resolution_ + grid_centre_pos_x_ - (width_ / 2.0) * resolution_ - laser_x, 2.0) +
                        ...
                        double angle = betweenPIandNegPI(
                            atan2(j * resolution_ + grid_centre_pos_y_ - (width_ / 2.0) * resolution_ - laser_y,
                                  i * resolution_ + grid_centre_pos_x_ - (width_ / 2.0) * resolution_ - laser_x) -
                            laser_yaw);
                        bool is_human;
                        ...
                        if (angle > scan.angle_min - scan.angle_increment / 2.0 and
                            ...
                            int closest_beam_idx = std::max(
                                0, std::min(static_cast<int>(scan.ranges.size() - 1), idx_without_bounds_check));
                            is_human = is_sample_human[closest_beam_idx];

                            // Processing the range value of the closest_beam to determine if it's a valid measurement
                            // or not. Sometimes it returns infs and NaNs that have to be dealt with
                            bool valid_measurement;
                            if (scan.range_min <= scan.ranges[closest_beam_idx] &&
                                scan.ranges[closest_beam_idx] <= scan.range_max) {
                                // This is a valid measurement.
                                valid_measurement = true;
                            } else if (!std::isfinite(scan.ranges[closest_beam_idx]) &&
                                       scan.ranges[closest_beam_idx] < 0) {
                                ...
                                // The sensor reported these measurements as valid, but they are discarded per the
                                // limits defined by minimum_range and maximum_range.
                                valid_measurement = false;
                            }
                            ...
                            if (valid_measurement) {
                                ...
                                if (dist > scan.range_max or dist > scan.ranges[closest_beam_idx] + ALPHA / 2.0 or
                                    fabs(angle_rel) > BETA / 2 or
                                    (!std::isfinite(scan.ranges[closest_beam_idx]) and dist > reliable_inf_range_)) {
                                    m_update = UNKNOWN;
                                } else if (scan.ranges[closest_beam_idx] < scan.range_max and
                                           fabs(dist_rel) < ALPHA / 2 and !is_human) {
                                    m_update = OBSTACLE;
                                } else {
                                    m_update = FREE_SPACE;
                                }
                            } else {
                                // Assume cells corresponding to erroneous measurements are either in freespace or
                                // unknown
