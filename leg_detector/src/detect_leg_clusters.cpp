...
public:
    DetectLegClusters() : Node("detect_leg_clusters")
    {
        ...
        this->declare_parameter("fixed_frame", rclcpp::ParameterValue(std::string("laser_frame")));
        ...
        this->declare_parameter("max_detected_clusters", rclcpp::ParameterValue(-1));
        ...
        this->scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
            scan_topic, default_qos, std::bind(&DetectLegClusters::laserCallback, this, std::placeholders::_1));
...
private:
    ...
    void laserCallback(const sensor_msgs::msg::LaserScan::SharedPtr scan)
    {
        ...
        if (use_scan_header_stamp_for_tfs_)
        {
            ...
            try
            {
                // lookupTransform() is a function of the Buffer class that looks up the transformation relationship
                // between two coordinate systems.
                // RCLCPP_INFO(this->get_logger(), "%s", fixed_frame_);
        ...
        else
        {
            ...
            // RCLCPP_INFO_STREAM(this->get_logger(), fixed_frame_);
            tf_time.now();
            transform_available = buffer_->canTransform(fixed_frame_, scan->header.frame_id, tf_time1);
        }
        ...
        else
        {
                 ...
                 cluster != processor.getClusters().end(); cluster++)
            {
                ...
                if (rel_dist < max_detect_distance_)
                {
                    ...
                    // RCLCPP_INFO_STREAM(this->get_logger(), probability_of_leg);
#else
                    // The forest->predict funciton has been removed in the latest versions of OpenCV so we'll do the
                    // calculation explicitly.
                    RCLCPP_INFO(this->get_logger(), "Checkout 6");
                    cv::Mat result;
                    forest->getVotes(cv::Mat(*tmp_mat), result, 0);
                    ...
                    if (probability_of_leg > detection_threshold_)
                    {
                        ...
                        // RCLCPP_INFO_STREAM(this->get_logger(), transform_successful_2);
                        if (transform_successful_2)
                        {
                            // Add detected cluster to set of detected leg clusters, along with its relative position to
                            // the laser scanner
                            ...
                            // RCLCPP_INFO(this->get_logger(),"%d, %d",new_leg.position.x, new_leg.position.y);
                        }
                    }
                }
            }
        ...
        for (std::set<leg_detector_msgs::msg::Leg>::iterator it = leg_set.begin(); it != leg_set.end(); ++it)
        {
            ...
            // RCLCPP_INFO(this->get_logger(),"%d, %d",m.pose.position.x, m.pose.position.y);
        ...
        RCLCPP_INFO(this->get_logger(), "still run");
    }
